...
function ProgramForm({ portfolioId, program, onSave, onCancel }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = { ...form, portfolio_id: portfolioId };
      program ? await client.put(`/programs/${program.id}`, payload) : await client.post('/programs', payload);
      onSave();
  ...
  return (
    <div style={styles.overlay}>
      <div style={styles.modal}>
        <h3 style={styles.modalTitle}>{program ? 'Edit Program' : 'New Program'}</h3>
...
export default function PortfolioDetail() {
  const { id } = useParams();
  const { data, loading, error, refetch } = useApi(`/portfolios/${id}`);
  ...
  const handleDeleteProgram = async () => { await client.delete(`/programs/${deleteProg.id}`); setDeleteProg(null); refetch(); };
  ...
  return (
    <div>
      <div style={styles.breadcrumb}><Link to="/portfolios" style={styles.breadLink}>Portfolios</Link> / {data.name}</div>
      <div style={styles.header}>
        <div style={{ flex: 1 }}>
          ...
          <div style={styles.progressWrap}>
            <ProgressBar value={data.percent_complete ?? 0} size="lg" label="Portfolio Completion" />
          </div>
      ...
      <div style={styles.section}>
        ...
        {data.programs?.length === 0 && <p style={styles.empty}>No programs yet.</p>}

        {/* Gantt view */}
        {progView === 'gantt' && (data.programs?.length ?? 0) > 0 && (
          <GanttChart
            items={data.programs}
            nameField="name"
          ...
          {data.programs?.map(prog => (
            <div key={prog.id} style={styles.card}>
              ...
              <div style={styles.cardProgress}>
                <ProgressBar value={prog.percent_complete ?? 0} size="sm" showLabel={false} />
                <span style={{ fontSize: '0.72rem', fontWeight: 700, color: (prog.percent_complete ?? 0) >= 75 ? '#16a34a' : (prog.percent_complete ?? 0) >= 40 ? '#d97706' : '#016D2D', minWidth: '2.4rem', textAlign: 'right' }}>
              ...
              {canEdit('programs') && (
                <div style={styles.cardActions}>
                  <button style={styles.editBtn} onClick={() => setEditProg(prog)}>Edit</button>
                  <button style={styles.deleteBtn} onClick={() => setDeleteProg(prog)}>Delete</button>
                </div>
              )}
            </div>
          ))}
      ...
      {(showProgForm || editProg) && <ProgramForm portfolioId={parseInt(id)} program={editProg} onSave={() => { setShowProgForm(false); setEditProg(null); refetch(); }} onCancel={() => { setShowProgForm(false); setEditProg(null); }} />}
      {deleteProg && <ConfirmDialog message={`Delete program "${deleteProg.name}"?`} onConfirm={handleDeleteProgram} onCancel={() => setDeleteProg(null)} />}

      {/* ── EVM ─────────────────────────────────────────────────── */}
      <EVMPanel parentType="portfolio" parentId={parseInt(id)} />
    </div>
  );
}
...
const styles = {
  ...
  card:       { background: '#f9fafb', borderRadius: '10px', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.4rem', border: '1.5px solid #e5e7eb' },
  cardHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' },
  cardTitle:  { fontWeight: 700, color: '#1d1d1d', textDecoration: 'none', fontSize: '0.98rem' },
  ...
  metaRow:    { display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '0.5rem', alignItems: 'center' },
  priorityTag:{ fontSize: '0.78rem', fontWeight: 700, padding: '3px 10px', borderRadius: '5px' },
  datePill:   { fontSize: '0.78rem', color: '#6b7280', background: '#f3f4f6', padding: '3px 10px', borderRadius: '5px' },
